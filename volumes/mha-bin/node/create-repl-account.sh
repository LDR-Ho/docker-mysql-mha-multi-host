...
"${mysql[@]}" <<-EOSQL
	GRANT REPLICATION SLAVE ON *.* TO "$MYSQL_REPL_NAME"@"10.126.144.%" IDENTIFIED BY "${MYSQL_REPL_PASSWORD:-}";
	GRANT ALL PRIVILEGES ON *.* TO "$MYSQL_MHA_NAME"@"10.126.144.%" IDENTIFIED BY "${MYSQL_MHA_PASSWORD:-}";
	FLUSH PRIVILEGES;
EOSQL
